...
async def update_threat_with_qmind_result(result: dict):
    ...
    if new_status == "CONFIRMED_THREAT" and _case_manager:
        ...
        if _honeygrid and lead_category in ("C2_Infrastructure", "Botnet_IP", "Malware", "Supply_Chain"):
            source_ip = result.get("source_ip", "")
            threat_id = UUID(result.get("threat_id", str(indicator)))
            task3 = asyncio.create_task(
                _honeygrid.deploy_honeypot(threat_id, source_ip, lead_category),
                name=f"honeygrid_{source_ip}"
            )
            task3.add_done_callback(handle_task_error)

        # Forward to SIEMs (CEF + Splunk) as background tasks
        cef_forwarder = get_cef_forwarder()
        ...
        splunk_hec = get_splunk_hec_client()
    ...
    # 4. WebSocket push to analyst dashboard (placeholder - no websocket manager yet)
    # TODO: Implement websocket_manager.broadcast_to_tenant when available
